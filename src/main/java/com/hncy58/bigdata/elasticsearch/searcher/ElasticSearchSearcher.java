...
public class ElasticSearchSearcher implements LuceneSearcher
{
	...
	@Override
	public Map<Object, Object> queryStatistics(String[] dbs, Query query, String... table) throws SearchEngineException 
	{
		...
		if (query.isSearchAll()) {
			...
			for (Criteria criteria: query.getCriterias()) {	
				...
				if (criteria.op == Criteria.operation.rangefilter) {
					fb = QueryBuilders.rangeQuery(criteria.key).includeLower(false)
										.from(criteria.values[0]).to(criteria.values[1]);
					filter.must(fb);
				}
